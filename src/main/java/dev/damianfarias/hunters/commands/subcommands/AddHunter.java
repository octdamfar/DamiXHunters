...
public class AddHunter implements SubCommand {
    ...
    public void execute(CommandSender s, String[] args, String cmdName) {
        if(args.length == 0){
            m().send(s, "addhunter-usage", Map.of("label", cmdName));
            return;
        }
        ...
        if(gm.isHunter(p.getUniqueId())){
            m().send(s, "addhunter-already-in", Map.of("player", args[0]));
            return;
        }

        if(gm.isRunner(p.getUniqueId())){
            m().send(s, "addhunter-is-runner", Map.of("player", args[0]));
            return;
        }

        DamiXHunters.getPlayerStateManager().setupPlayerAsHunter(p);
        m().send(s, "addhunter-successfully", Map.of("player", args[0]));
    }

    @Override
    public List<String> tabComplete(CommandSender s, String[] args) {
        return args.length == 1 ? Arrays.stream(Bukkit.getOfflinePlayers())
                .filter(player -> !DamiXHunters.getGameManager().getHunters().contains(player.getUniqueId()) && !DamiXHunters.getGameManager().getRunners().contains(player.getUniqueId()))
