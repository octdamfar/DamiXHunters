...
public class AddRunner implements SubCommand {
    @Override
    ...
    public void execute(CommandSender s, String[] args, String cmdName) {
        if(args.length == 0){
            m().send(s, "addrunner-usage", Map.of("label", cmdName));
            return;
        }
        ...
        if(gm.isRunner(p.getUniqueId())){
            m().send(s, "addrunner-already-in", Map.of("player", args[0]));
            return;
        }

        if(gm.isHunter(p.getUniqueId())){
            m().send(s, "addrunner-is-hunter", Map.of("player", args[0]));
            return;
        }

        DamiXHunters.getPlayerStateManager().setupPlayerAsRunner(p);
        m().send(s, "addrunner-successfully", Map.of("player", args[0]));
    }

    @Override
    public List<String> tabComplete(CommandSender s, String[] args) {
        return args.length == 1 ? Arrays.stream(Bukkit.getOfflinePlayers())
                .filter(player -> !DamiXHunters.getGameManager().getRunners().contains(player.getUniqueId()) && !DamiXHunters.getGameManager().getHunters().contains(player.getUniqueId()))
